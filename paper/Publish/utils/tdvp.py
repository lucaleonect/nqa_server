...
def validate_build_parametric_gradient_estimator_args(
    variational_quantum_state,
    local_hamiltonian: Callable[[jnp.ndarray, jnp.ndarray, CNumber], CNumber],
    prefactor: CNumber,
    p_inv_rcond: float,
    diag_shift: float,
    method: str,
    ...
    # if not isinstance(variational_quantum_state, VariationalQuantumState):
    #     raise TypeError(
    #         f"variational_quantum_state is expected to be an instance of VariationalQuantumState, got {type(variational_quantum_state)} instead."
    #     )

    if not callable(local_hamiltonian):
        raise TypeError(f"local_hamiltonian must be a callable, got {type(local_hamiltonian)} instead.")

    if not isinstance(prefactor, CNumber):
        raise TypeError(f"prefactor must be a CNumber number, got {type(prefactor)} instead.")

    if not isinstance(p_inv_rcond, float):
        raise TypeError(f"p_inv_rcond must be a float, got {type(p_inv_rcond)} instead.")

    if not p_inv_rcond > 0:
        raise ValueError(f"p_inv_rcond must be positive, got {p_inv_rcond} instead.")

    if not isinstance(diag_shift, float):
        raise TypeError(f"diag_shift must be a float, got {type(diag_shift)} instead.")

    if not isinstance(method, str):
        raise TypeError(f"method must be a string, got {type(method)} instead.")

    if not method in VALID_TDVP_METHODS:
        raise ValueError(f"method must be one of {VALID_TDVP_METHODS}, got {method} instead.")

    if not isinstance(return_aux, bool):
        raise TypeError(f"return_aux must be a bool, got {type(return_aux)} instead.")
...
def build_parametric_gradient_estimator(
...
) -> Callable[[jnp.ndarray, jnp.ndarray, CNumber], CNumber]:
    """
    ...
    Args:
        variational_quantum_state (VariationalQuantumState): Variational quantum state describing the system
    ...
    Returns:
        Callable[[jnp.ndarray, jnp.ndarray, CNumber], CNumber]: Function that estimates the gradient of the energy expectation value
    ...
    validate_build_parametric_gradient_estimator_args(
        variational_quantum_state=variational_quantum_state,
        local_hamiltonian=local_hamiltonian,
        prefactor=prefactor,
        p_inv_rcond=p_inv_rcond,
        diag_shift=diag_shift,
        method=method,
        return_aux=return_aux,
    )

    logpsi = variational_quantum_state.logpsi
    is_holomorphic = variational_quantum_state.is_holomorphic
    dtype = variational_quantum_state.dtype

    _num_samples = variational_quantum_state.num_samples
    _num_params = variational_quantum_state.params.size
    ...
    def minsr_estimate_gradients(
        ...
        Args:
            ...
            couplings (CNumber): couplings constant of the local Hamiltonian
        ...
        gradients = prefactor * d_logpsi_dag @ jnp.linalg.pinv(T_matrix, rcond=p_inv_rcond) @ local_energies
