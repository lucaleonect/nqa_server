...
parser.add_argument(
    ...
    action=argparse.BooleanOptionalAction,
    default=False,
    help="Use cRBM of Carleo and Troyer 2017 paper instead of Deep Boltzmann Quantum State. If passed, the number of hidden layers will be overridden to 1.",
)
...
parser.add_argument(
    ...
    action=argparse.BooleanOptionalAction,
    default=False,
    help="Whether to use single spin flip Metropolis-Hastings sampling instead of block Gibbs sampling. Default is False.",
)
...
from utils.variational_quantum_states.neural_quantum_states import cRBM
...
def main():
    ...
    # Handle h_vector: check for conflicting options, then set appropriately
    ...
    if args.cRBM:
        print(f"--cRBM passed, using cRBM with {dbqs_units_per_layer} hidden units.")
        print(f"Falling back to MH sampler, as cRBM does not support Gibbs sampling.")
        if args.dbqs_num_hidden_layers != 1:
            raise ValueError("cRBM only supports one hidden layer, please set --dbqs_num_hidden_layers=1")
        vqs = cRBM(
            num_spins=num_spins,
            num_hidden=dbqs_units_per_layer,
            prngkey=tempkey,
            num_samples=args.mcmc_num_samples,
            num_thermalization_steps=args.mcmc_num_thermalization_steps,
            num_sweep_steps=args.mcmc_num_sweep_steps,
    ...
    # region: Override gibbs sampler if requested
    if args.mcmc_metropolis_hastings:
        p_ratio_fn = jax.jit(
            lambda params, s, sp: jnp.exp(2 * np.real(vqs.logpsi(params, s) - vqs.logpsi(params, sp)))
        )
        _generate_samples, _update_samples = build_mcmc_sampler(
        ...
        vqs.generate_samples = jax.jit(_generate_samples)
        vqs.update_samples = jax.jit(_update_samples)
    # endregion
    # endregion
    ...
    if args.transverse_field_strength is not None:
        local_energy_target = lambda params, spins: local_energy_z_target(
            params, spins
        ) + args.transverse_field_strength * _local_energy_sigma_x(params, spins)
    else:
        local_energy_target = local_energy_z_target

    local_energy_annealing = (
        lambda params, spins: _local_energy_sigma_x(params, spins) * args.vqa_annealing_field_scale
    )
    if args.vqa_use_catalyst:
        _local_energy_catalyst = vqs.local_energy_sigma_y
        local_energy_catalyst = (
            lambda params, spins: _local_energy_catalyst(params, spins) * args.vqa_catalyst_field_scale
        )
    else:
